...
verify_trade_sequence() {
    ...
    # Get recent trades
    local trades
    trades=$(./invm trades "$((num_expected + 1))")
    echo "Recent trades:"
    echo "$trades"
...
clear_book() {
    ...
    book_data=$(./invm book 9999)
    while IFS=',' read -r type side price quantity; do
        if [[ "$type" == "level" ]]; then
            if [[ "$side" == "ask" ]]; then
                ./invm buy "$quantity@$price"
            else
                ./invm sell "$quantity@$price"
            fi
        fi
    done <<< "$book_data"

    # Verify book is clear
    if [[ -n $(./invm book 10) ]]; then
...
./invm sell 10@100
show_book
verify_trade_sequence "./invm buy 10@100" \
    "10" "100" "bid"
show_book
...
./invm sell 20@101
show_book
verify_trade_sequence "./invm buy 10@101" \
    "10" "101" "bid"
show_book
...
./invm sell 5@104
show_book
verify_trade_sequence "./invm buy 15@104" \
    "5" "104" "bid" \
    "5" "103" "bid" \
    "5" "102" "bid"
show_book
...
./invm sell 5@105
show_book
verify_trade_sequence "./invm buy 10@105" \
    "5" "105" "bid" \
    "5" "105" "bid"
show_book
...
./invm sell 1@106
show_book
verify_trade_sequence "./invm buy 1@106" \
    "1" "106" "bid"
show_book
